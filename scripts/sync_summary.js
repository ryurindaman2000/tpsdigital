...
async function syncSummary() {
  console.log('[INFO] Sinkronisasi Dokumen stats/summary dari seluruh suara di Firestore...');
  
  const [uSnap, cSnap, vSnap] = await Promise.all([
  ...
  cSnap.forEach(d => candidatesList.push({ id: d.id, ...d.data() }));
  candidatesList.sort((a, b) => (Number(a.candidateNumber) || 0) - (Number(b.candidateNumber) || 0));

  const votesList = [];
  vSnap.forEach(d => votesList.push(d.data()));
  ...
  console.log('✅ [SUCCESS] Dokumen stats/summary Berhasil Diperbarui secara Presisi!');
